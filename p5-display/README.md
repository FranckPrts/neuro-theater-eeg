...
**Collective spider plot** (`spider-plot-collective.p5`) uses a **single active parameter** (`plot_0_axis_*`): each branch is one headset, and the radar always reads that one mapping. There is **no** “Overlays to visualize” control. Stream **chips** list every suffix seen on the network (union); clicking one (e.g. `alphaNorm`) maps `/{device}/alphaNorm` onto branches for every device that publishes it. Branch rows are manual OSC dropdowns only (fixed branch colors in the sketch). Saved mappings from older multi-group layouts are migrated into `plot_0` on load when `plot_0` is empty.
