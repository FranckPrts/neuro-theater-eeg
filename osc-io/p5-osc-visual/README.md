...
- **Scene:** three collapsible blocks:
  ...
  3. **OSC mapping** — one row per **input** for the active scene (from [`registry.json`](public/p5-scenes/registry.json) for most scenes). Each OSC row is a dropdown of addresses seen so far (first numeric argument → `data[inputId]`). **`spider-plot.p5`** is special: the server derives **`plotCount`** from the line `const NUM_OVERLAY_PLOTS = <n>;` in that file; the rail shows **5 OSC dropdowns + 1 HEX text field per plot**, plus **radius mode** (relative vs absolute), **mean**, and **max deviation** for absolute scaling. HEX colors persist under `nt.p5osc.spiderHex`. **Mapping CSV** — load/save the active scene’s mapping from `public/p5-mapping/*.csv` via the strip (see below).

**Persistence:** `localStorage` keys `nt.p5osc.panelMode`, `nt.p5osc.activeScene`, `nt.p5osc.mappings` (per-scene OSC maps), **`nt.p5osc.spiderHex`** (per-plot colors for the spider scene), **`nt.p5osc.spiderDrawOverlays`** (how many spider layers to draw, 1…rail max), and **`nt.p5osc.spiderRadius`** (spider radius mode + absolute mean/max, per scene file).
...
Each scene script is written in **global p5 mode** (`setup` / `draw`, global `data` object). The host maps OSC into `data` keys that match each input **`id`**, and for the spider scene adds `data.plot_<p>_color` strings, `data.__plotCount`, `data.__radiusMode`, `data.__absoluteMean`, and `data.__absoluteMax` from the rail.
...
| `inputId`  | Registry input `id`, or spider ids `plot_<n>_axis_<0-4>`, `plot_<n>_color`, `__plotCount`, `__radiusMode`, `__absoluteMean`, `__absoluteMax`. |
...
- `server.js` — static files from `public/`, WebSocket on `/ws`, OSC UDP listener, **`GET /api/p5-scenes`**, mapping CSV **`GET/POST /api/p5-mappings`**.
